const createSingleRecipeWidgetLoginPage = (recipe) =>
    ` <li onclick="saveToLocalStorage(${recipe.id},'${recipe.recipeName}', '${recipe.imageCover}',
                                   ...
                                   `;
const createSingleCategoryItemLoginPage = (item) =>
    ` <li>
...
const callLatestRecipeWidgetLoginPageApi = async () => {
    ...
    for (var item of data) {
        count++;
        if (count >= 8) {
            let element = createSingleRecipeWidgetLoginPage(item);
            $("#list-latest-recipe-widget-loginpage").append(element);
        }
    }
};
const callListCategoryItemLoginPageApi = async () => {
    ...
    for (var item of data) {
        for (var cateItem of item.listCategoryItem) {
            let element = createSingleCategoryItemLoginPage(cateItem);
            $("#list-category-item-loginpage").append(element);
        }
    }
};
const callPopularLoginPageApi = async () => {
    ...
    for (var item of data) {
        count++;
        let element = createSingleRecipeWidgetLoginPage(item);
        $("#list-popular-recipe-widget-loginpage").append(element);
        if (count >= 5) {
            break;
        }
    }
